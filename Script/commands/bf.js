...
module.exports.config = {
  ...
  version: "1.0.0",
  hasPermssion: 0,
  credits: "Ullash api rasin (edited for bf)",
  description: "সিঙ্গেলদের জন্য bf জেনারেটর",
  usePrefix: false,
  commandCategory: "fun",
  usages: "/need bf",
  cooldowns: 20,
...
module.exports.run = async function ({ api, event }) {
  try {
    ...
    // এখানে gf এর জায়গায় bf
    const fullUrl = `${apiUrl}/api/rasin/bf?apikey=${apiKey}`;

    const res = await axios.get(fullUrl);
    const title = res.data.data.title;
    const imgUrl = res.data.data.url;

    const imgPath = path.join(__dirname, "cache", `${event.senderID}_bf.jpg`);
